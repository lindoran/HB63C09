;**************************************************************************
;6809 EXTENDED BASIC
;SOURCE UNKNOWN BUT APPEARS TO BE A SUBSET OF MICROSOFT
;  EXTENDED BASIC FOR THE TANDY COLOR COMPUTER. POSSIBLY
;  MODIFIED BY GRANT SEARLE. -- JQ
;**************************************************************************
; VERSION 1.1
; 2021-0420:	JQ	MODIFIED TO RUN AS A LANGUAGE MODULE UNDER MON09.
;					ADDED 'BYE' COMMAND TO RETURN TO MON09. WHEN THE MON09
;					 EQUATE IS SET, THE 'BYE' COMMAND USES TOKEN $83 AND
;					 REPLACES "'", THE SHORT FORM OF THE 'REM' COMMAND.
;		TODO:		BUG: "HEX$" (HEXDOL) COMMAND RETURNS A SYNTAX ERROR.
;**************************************************************************

DEBUG	EQU	0						;SET TO '1' TO BUILD IN RAM
MON09	SET	1						;SET TO '1' TO INCLUDE AS PART OF MON09
VERSION	EQU	11						;VERSION X 100

ROMSTART	EQU	$C000				;START OF ROM
RAMSTART	EQU	$0000				;START OF RAM
 IFEQ MON09
RAMEND		EQU	$9FFF				;END OF RAM
 ELSE
RAMEND		EQU	$9FFF				;END OF RAM (WITH MON09)
 ENDC

	IF DEBUG
CODESTART	EQU	$1000				;START OF THIS PROGRAM IN RAM
MON09		SET	1					;SET TO '1' TO INCLUDE AS PART OF MON09
	ELSE
CODESTART	EQU	$C000				;START OF THIS PROGRAM IN ROM
	ENDC

DATASTART	EQU	RAMSTART			;START OF DATA IN RAM

 IFNE MON09			;USE WITH MON09
	INCLUDE	../../mon09/V36/mon09v36_API.inc
 ENDC				;USE WITH MON09

;== ACIA CONSTANTS ====================================================
UART	EQU  $A000				;UART BASE ADDRESS
RECEV	EQU  UART+1				;UART RECEIVE REGISTER
TRANS	EQU  UART+1				;UART TRANSMIT DATA REGISTER
USTAT	EQU  UART				;UART STATUS REGISTER
UCTRL	EQU  UART				;UART CONTROL REGISTER
...
;
; NOTE: Code gets copied to RAM from ROM around LA123
;
GETNCH    RMB  6
GETCCH    RMB  1
CHARAD    RMB  5
...
STACK     RMB  64             ;	RESERVE STACK SPACE
...
;******************************************************************
; KEYIN: THIS ROUTINE GETS A KEYSTROKE FROM THE UART IF A CHARACTER
;        IS READY. IT RETURNS ZERO TRUE IF THERE WAS NO KEY DOWN.
;******************************************************************
KEYIN     LDA  USTAT		;FETCH UART STATUS
          BITA #RDRF		;CHARACTER READY?
          BEQ  NOCHAR		;SKIP AND EXIT
          LDA  RECEV		;FETCH THE CHARACTER
          ANDA #$7F			;STRIP 'PARITY' BIT
          RTS				;RETURN TO CALLER
NOCHAR    CLRA				;A=0 -> NO CHARACTER
          RTS				;RETURN TO CALLER
;
;******************************************************************
; PUTCHR: CONSOLE OUT
;******************************************************************
PUTCHR    BSR  WAITACIA		;TEST UART STATUS REGISTER
          PSHS A			;SAVE A
          CMPA #CR			;IS IT CARRIAGE RETURN?
          BEQ  NEWLINE		;YES
          STA  TRANS		;SEND CHARACTER
...
NEWLINE   CLR  LPTPOS		;RESET CHARACTER COUNTER
          BSR  WAITACIA		;TEST UART STATUS REGISTER
          LDA  #13			;CARRIAGE RETURN
          STA  TRANS		;SEND CHARACTER
          BSR  WAITACIA		;TEST UART STATUS REGISTER
          LDA  #10			;DO LINEFEED AFTER CR
          STA  TRANS		;SEND CHARACTER
PUTEND    PULS A			;RESTORE A
          RTS				;RETURN TO CALLER

WAITACIA  PSHS A
WRWAIT    LDA  USTAT		;FETCH UART STATUS
          BITA TDRE			;TX DATA REGISTER EMPTY?
          BEQ  WRWAIT		;NO, SO LOOP
          PULS A			;RESTORE A
          RTS				;RETURN TO CALLER
;
;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; COLD START ENTRY
;;;;;;;;;;;;;;;;;;;;;;;;;;
BACDST    LDX  #DATASTART     ; CLEAR INTERNAL DATA IN RAM
LA077     CLR  ,X+            ; CLEAR BYTE, INCREMENT X
          CMPX #STACK         ; END OF INTERNAL DATA?
          BNE  LA077          ; KEEP GOING TILL FINISHED
          LDX  #PROGST        ; SET X TO START OF PROGRAM SPACE
...
LA084     LDA  2,X            ; LOOK FOR END OF MEMORY
          ...
          CMPX #RAMEND        ; END OF RAM REACHED?
          BEQ  LA093          ; BRANCH IF YES
...
L8031     STU  ,X++           ; STORE 'FC' ERROR AT USR ADDRESSES
 ...
 IFEQ MON09				;IF NOT USING MON09
;******************************************************************
; INITIALIZE ACIA
;******************************************************************
INTIACIA
          LDA  #CTRL1			;MASTER RESET
          STA  UCTRL			;SET REGISTER
          LDA  #CTRL2			;115.2K BAUD @ 8N1
          STA  UCTRL			;SET REGISTER
 ENDC					;IF NOT USING MON09
;******************************************************************
...
;MODS BY JQ
LA147     FCB  CR,LF
          FCC  "6809 EXTENDED BASIC FOR 6809SBC"
          FCB  CR
          FCC  " VERSION "
          ...
          FCB  CR
          FCC  " (C)2021 MODS FOR 6809SBC BY Johnny Quest"
LA156     FCB  CR,CR
LA165     FCB  $00
...
LA39A     JSR  CONSIN          ; GO GET A CHARACTER FROM CONSOLE IN
          CMPA #BS            ; BACKSPACE
          BNE  LA3B4          ; NO
...
TOK_GO    EQU  $81
 ...
 IFNE MON09		;IF USING WITH MON09
          FCC  'BY'
          FCB  'E'+$80        ; 83
 ELSE
          FCB  ''+$80         ; 83
...
TOK_REM   EQU  ((*-CMD_TAB)/2)+$7F
 ...
 IFNE MON09		;IF USING WITH MON09
TOK_SNGL_Q EQU TOK_REM
          FDB  BYE            ; 83
 ELSE
          FDB  REM            ; 83 (')
TOK_SNGL_Q EQU  ((*-CMD_TAB)/2)+$7F
 ENDC			;IF USING WITH MON09
...
*
* BYE	;RETURN TO MON09
BYE       JMP  MON09_RESET	;reset to MON09
 ENDC		;IF USING WITH MON09
*
* END
...
*
* GET A KEYSTRKE
L8687     JSR  CONSIN          ; CALL CONSOLE IN : DEV NBR=SCREEN
