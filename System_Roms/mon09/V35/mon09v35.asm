...
;******************************************************************************
;MODS BY JQ
VERSION		EQU	35		VERSION TIMES 10
JQ_DETMCU	EQU	1		SET TO '1' TO ENABLE DETECTION OF MCU TYPE
JQ_NMIRAM	EQU	1		SET TO '1' TO ENABLE NMI HANDLER IN RAM
JQ_ILLOP	EQU	1		SET TO '1' TO ENABLE 6309 ILLOP HANDLER
JQ_LDFIX	EQU	0		SET TO '1' TO ENABLE PROPER EOF HANDLING DURING LOAD
...
FPBASSTRT	EQU	$BE00				START ADDRESS OF FP BASIC IN ROM
FPBASEND	EQU	FPBASSTRT+$2600		END ADDRESS OF FP BASIC IN ROM
;
;== HARDWARE CONSTANTS =================================================
CODE		EQU	$E400	MON09 code goes here
RAMSTRT		EQU	$0000	START OF RAM
RAMEND		EQU	$8000	END OF RAM

ROMSTRT		EQU	$8800	START OF ROM
...
;
;== 6850 ACIA CONSTANTS ===============================================
SER1	EQU	$8000		Serial Port 1 (Console)
SER2	EQU	$8000		Serial Port 2 (Aux)
...
;	INCLUDE A DATE OF BUILD STRING
	...
	FCB		$0A,$0D
	FCC		'(C)2021 Johnny Quest'
	FCB		$0A,$0D
	FCC		' Modified and Enhanced for 6809SBC by JQ'
;	FCB		$0A,$0D,$FF
	FCB		$FF
;
;------------------------------------------------------------
...
MONITOR1
	...
	;FCB		$11,$3D,$01	OP CODES FOR 'LDMD #$01'
...
JUMPFPBAS
	...
	JMP		FPBASSTRT	JUMP TO FORTH COLD START
...
DLEOF
	IFNE JQ_LDFIX
	JMP		DLEOF_E			FINISH LAST EOF RECORD
	ELSE
...
GETB3
	...
	IFNE JQ_LDFIX		not functional!
;*
;* EOF DETECTED, CONTINUE FETCHING BYTES TILL A CR OR LF
;*  IS RECEIVED THEN RETURN TO CALLER
;*
DLEOF_E
	LBSR	GETCHR			Get ASCII character
	CMPA	#$0D			Is it a CR character?
	BEQ		DLEOF_E1		Yes, exit
	CMPA	#$0A			Is it a LF character?
	BEQ		DLEOF_E1		Yes, exit
	BRA		DLEOF_E			Loop till CR or LF recieved
DLEOF_E1
	RTS						RETURN TO CALLER
	ENDC
;*
